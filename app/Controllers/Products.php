    ...
    public function save()
    {
        ...
        if ($this->request->getMethod() == "post") {
            ...
            if ($this->request->getVar("barcode_no", FILTER_SANITIZE_STRING) == "") {
                ...
                reCheckBarcode: //label for the go to statement
                ...
                if (null !== $this->request->getFile("img")) {
                    ...
                    if ($file->isValid() && !$file->hasMoved()) {
                        ...
                        if ($file->move(FCPATH . 'public/uploads/products', $fileName)) {
                            ...
                            if ($this->productsModel->updateProductImg($fields['barcode'], (string)$this->store_id, ['image' => $path])) {
        ...
        switch ($method) {
            ...
            default:
                ...
                foreach ($data['categories'] as $key => $category) {
                    $categorySalesAmount = $this->storeModel->getSalesAmountByCategory($this->store_id, $category["cat_id"]);
                    if (is_array($categorySalesAmount)) {
                        foreach ($categorySalesAmount as $eachAmount) {
                            $amountTotal += (float)$eachAmount["amount"];
                        }
                        $data['categories'][$key]["amountTotal"] = $amountTotal;
                        $amountTotal = 0;

                        $categorySalesQuantity = $this->storeModel->getSalesQuantityByCategory($this->store_id, $category["cat_id"]);
                        foreach ($categorySalesQuantity as $eachQuantity) {
                            $quantityTotal += (float)$eachQuantity["quantity"];
                        }
                        $data['categories'][$key]["quantityTotal"] = $quantityTotal;
                        $quantityTotal = 0;
                    }
                }
